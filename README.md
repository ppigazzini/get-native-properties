[![CI](https://github.com/ppigazzini/get-native-properties/actions/workflows/ci.yml/badge.svg)](https://github.com/ppigazzini/get-native-properties/actions/workflows/ci.yml) [![CI lint](https://github.com/ppigazzini/get-native-properties/actions/workflows/lint.yml/badge.svg)](https://github.com/ppigazzini/get-native-properties/actions/workflows/lint.yml) [![Platform Smoke](https://github.com/ppigazzini/get-native-properties/actions/workflows/platform-smoke.yml/badge.svg)](https://github.com/ppigazzini/get-native-properties/actions/workflows/platform-smoke.yml)  [![Shell coverage](https://github.com/ppigazzini/get-native-properties/actions/workflows/shell-coverage.yml/badge.svg)](https://github.com/ppigazzini/get-native-properties/actions/workflows/shell-coverage.yml)
...
If you don't use `uv`, you can run them with plain Python:
...
In addition to behavioral unit tests, this repo has a dedicated shell coverage workflow that produces a line coverage report for the POSIX `sh` script.

Key points:

- Coverage is collected with **kcov** in **Docker** (so CI doesn’t depend on the runner OS packages).
- The production script stays POSIX `sh`; the coverage run forces the kcov **bash parser** to make tracing reliable.
- The harness runs many curated OS/arch/feature cases and merges them into one report.

### Running coverage locally

The coverage harness is:

- `scripts/kcov_get_native_properties.sh`

It writes a merged HTML report under:

- `coverage/kcov/index.html`
...
docker run --rm --platform linux/amd64 \
  ...
  -v "$PWD:/work" -w /work \
  kcov/kcov:latest-alpine \
  sh -lc 'apk add --no-cache bash && sh scripts/kcov_get_native_properties.sh'
```

### Why some regions are excluded

kcov can count some *data* blocks as “instrumented lines” even though they aren’t executable shell logic, notably:

- Table data in `<<'EOF' ... EOF` here-docs used by the selector
- Multi-line `awk ' ... '` program bodies

The harness excludes these regions so the reported percentage reflects executable decision logic rather than embedded table/program text.
...
This repo has four GitHub Actions workflows:
...
- **Shell coverage** (`.github/workflows/shell-coverage.yml`): kcov-based line coverage report for the shell script

## License

This repository is dual-licensed on a per-file basis:

- Files originating from the Stockfish project are licensed under **GPL-3.0** (see `COPYING`), per the upstream project's license.
