...
class TestGetNativeProperties(unittest.TestCase):
    ...
    def test_linux_fixtures(self) -> None:
        ...
        cases = [
            ...
            ("x86_64", "x86_64_popcnt_only.cpuinfo", "x86-64\n"),
            ...
            ("x86_64", "x86_64_sse3_popcnt_sse3.cpuinfo", "x86-64-sse3-popcnt\n"),
            ...
            ("ppc64", "ppc64_altivec_cpu_unparseable.cpuinfo", "ppc-64-altivec\n"),
            ("ppc64le", "ppc64_altivec_cpu_numeric_only.cpuinfo", "ppc-64-vsx\n"),
    ...
    def test_linux_unreadable_cpuinfo_falls_back_safely(self) -> None:
        """Unreadable cpuinfo should not crash and should pick a safe tier."""
        out = run_script(
            uname_s="Linux",
            uname_m="x86_64",
            cpuinfo=FIXTURES / "does_not_exist.cpuinfo",
        )
        assert out == "x86-64\n"  # noqa: S101

    def test_linux_x86_vendor_only_cpuinfo_falls_back(self) -> None:
        """Vendor/family-only cpuinfo should still fall back safely."""
        out = run_script(
            uname_s="Linux",
            uname_m="x86_64",
            cpuinfo=FIXTURES / "x86_64_empty_vendor_only.cpuinfo",
        )
        assert out == "x86-64\n"  # noqa: S101

    def test_linux_arm_falls_back_to_uname_when_cpuinfo_missing(self) -> None:
        """ARM level should fall back to uname encoding when cpuinfo is missing."""
        out = run_script(
            uname_s="Linux",
            uname_m="armv7l",
            cpuinfo=FIXTURES / "does_not_exist.cpuinfo",
    ...
    def test_linux_armv5_falls_back_to_uname_when_cpuinfo_missing(self) -> None:
        """armv5* should map to a safe 32-bit tier when cpuinfo is missing."""
        out = run_script(
            uname_s="Linux",
            uname_m="armv5tel",
            cpuinfo=FIXTURES / "does_not_exist.cpuinfo",
    ...
    def test_linux_armv6_falls_back_to_uname_when_cpuinfo_missing(self) -> None:
        """armv6* should map to a safe 32-bit tier when cpuinfo is missing."""
        out = run_script(
            uname_s="Linux",
            uname_m="armv6l",
            cpuinfo=FIXTURES / "does_not_exist.cpuinfo",
    ...
    def test_linux_armv8l_falls_back_to_uname_when_cpuinfo_missing(self) -> None:
        """armv8l should map to an armv7 tier when cpuinfo is missing."""
        out = run_script(
            uname_s="Linux",
            uname_m="armv8l",
            cpuinfo=FIXTURES / "does_not_exist.cpuinfo",
    ...
    def test_linux_arm_processor_only_v7_is_detected(self) -> None:
        """ARMv7 should be detected from the Processor: field when present."""
        out = run_script(
            uname_s="Linux",
            uname_m="arm",
            cpuinfo=FIXTURES / "arm_processor_only_v7.cpuinfo",
        )
        assert out == "armv7-neon\n"  # noqa: S101

    def test_linux_arm_unknown_neon_defaults_to_neon_tier(self) -> None:
        """If ARM level can't be determined but neon is present, pick neon tier."""
        out = run_script(
            uname_s="Linux",
            uname_m="arm",
            cpuinfo=FIXTURES / "arm_unknown_neon_only.cpuinfo",
        )
        assert out == "armv7-neon\n"  # noqa: S101

    def test_linux_arm_unknown_no_neon_defaults_general_32(self) -> None:
        """If ARM level can't be determined and neon is absent, pick generic 32-bit."""
        out = run_script(
            uname_s="Linux",
            uname_m="arm",
            cpuinfo=FIXTURES / "arm_unknown_no_neon.cpuinfo",
        )
        assert out == "general-32\n"  # noqa: S101
    ...
    def test_linux_unknown_machine_invalid_bits_defaults_64(self) -> None:
        """Invalid bitness should default to 64-bit generic."""
        out = run_script(uname_s="Linux", uname_m="mysterycpu", bits="wat")
        assert out == "general-64\n"  # noqa: S101
